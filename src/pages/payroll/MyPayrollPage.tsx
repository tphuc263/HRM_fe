...
import { Wallet, Calendar, AlertCircle, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Download } from 'lucide-react';
...
export default function MyPayrollPage() {
  ...
  const handleDownloadPdf = (payrollId: number, month: string) => {
    const element = document.getElementById(`payroll-card-${payrollId}`);
    if (element) {
      const opt = {
        margin: 0.5,
        filename: `phieu_luong_thang_${month}.pdf`,
        image: { type: 'jpeg' as const, quality: 0.98 },
        html2canvas: { scale: 2, useCORS: true },
        jsPDF: { unit: 'in' as const, format: 'a4' as const, orientation: 'portrait' as const }
      };

      // Temporarily add a class for PDF specific styling if needed
            ...
            <>
              <div className="grid gap-6">
                {payrolls.map(payroll => (
                  <div key={payroll.id} id={`payroll-card-${payroll.id}`} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative">
                    <div className="bg-gray-50 border-b p-4 flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-3">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleDownloadPdf(payroll.id, payroll.month)}
